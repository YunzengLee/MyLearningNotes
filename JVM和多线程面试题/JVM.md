...
- 方法区
  ...
  - 运行时常量池
    ...
    方法区的一部分。class文件中除了类的版本、字段、方法、接口等描述信息外，还有常量池信息（存放编译期生成的字面量和符号引用）
    ...
    - Eden  1/3 * 8
      ...
      - Eden区放满后GC，GC分两种
        - Young GC（MinorGC）：新生代产生的GC，执行时间相当短，效率相当高
          - Eden区满后执行GC，引用计数为0的清除，存活的对象放到From Survivor区
          - Survivor区分为From和To
...
通过类加载器将.class文件从磁盘读到内存

#### 2. 连接

1. 验证：验证字节码文件的正确性
2. 准备：给类的静态变量分配内存并赋予默认值。
3. 解析：类装载器装入类所引用的其他所有类。也就是静态链接过程，将类的符号引用转为直接引用
...
加载JRE的核心类库，如lib目录下的jar包和类。这个是用c++实现的，因为有一些c语言的包要加载。

#### 扩展类加载器

ExtensionClassLoader

负责加载JRE扩展目录lib/ext中的jar包和类
...
- 强引用（默认的，最普遍，对象在虚拟机生成时的引用，不会被垃圾回收）
- 软引用（做缓存来使用的，如果内存不够就会回收，内存够用即使计数为0也不会被回收）
- 弱引用（无论内存是否足够，计数为0一定会被回收）
- 虚引用（不会决定对象的生命周期，相当于没有引用，任何时候都可能被回收）
...
- 注意：该收集器是多线程并发而不是并行，尽管如此，与前者相比还是STW时间减少了很多（但还是存在STW）
- 新生代复制算法，老年代标记整理算法。
...
- 关注点是吞吐量（高效的利用CPU），CMS等垃圾收集器关注点更多的是用户线程的停顿时间（提高用户体验）
- 新生代复制算法，老年代标记整理算法。
